...
class _FavoriteScreenState extends State<FavoriteScreen> {
  ...
  Future<void> fetchTickets() async {
    final response = await http.get(Uri.parse(
        'https://ticketresellapi-ckhsduaycsfccjek.eastasia-01.azurewebsites.net/api/Favorite/get-all-favorite/${UserManager().id}'),
        headers: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: Text('All Favorite', style: Theme.of(context).textTheme.headlineMedium),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            children: [
              TGridLayout(itemCount: tickets.length, itemBuilder: (_, index) => TProductCardVerticalFav(ticket: tickets[index],))
            ],
          ),
        ),
      ),
